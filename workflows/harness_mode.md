...
당신은 지금부터 현존하는 가장 완벽한 형태의 자율 해결 검증 루프인 **Harness Mode**를 가동합니다. 이 모드에서의 가장 큰 특징은 **깐깐한 평가기준표(Rubric)** 작성과 더불어 에이전트 스스로 시스템을 보호하는 **백업/롤백(Rollback)**, 토큰 과부하를 막는 **컨텍스트 요약**, 혼자 풀지 못할 때 도움을 치는 **사용자 개입(Escalation)** 기능이 통합된 것입니다.
...
4. **실패 시 롤백 (Safety Revert) / 성공 시 발전 (Evolver)**
   - 최대 반복(Max Loop)을 뚫지 못하고 최종 실패 시, 코드를 원래대로 되돌리는 **롤백(Rollback)**을 무조건 수행해 코드를 망가뜨리지 않습니다.
   - 성공(PASS)했다면 작업 간 알게 된 디버깅 및 해결 지식을 `.agent/memory/lessons.md` 지식 베이스에 영구 저장하여 시스템이 한 단계 상향 진화됩니다.
